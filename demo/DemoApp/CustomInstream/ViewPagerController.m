...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        ...
        [bezierPath moveToPoint:CGPointMake(20.0, CGRectGetHeight(rect) - 1.0)];
        [bezierPath addLineToPoint:CGPointMake(140, CGRectGetHeight(rect) - 1.0)];
...
- (void)layoutSubviews {
    ...
    // custom
    UIView *tabLine = [self.tabsView viewWithTag:kTabLineTag];
    tabLine.frame = CGRectMake(0.0, 0.0, self.tabsView.contentSize.width ,[self.tabHeight floatValue]);
...
- (UIColor *)indicatorColor {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:indicatorColorForTabAtIndex:)]) {
        _indicatorColor = [self.delegate viewPager:self indicatorColorForTabAtIndex:_activeTabIndex];
    }
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        // custom
        TabLine *tabLine = [[TabLine alloc]initWithFrame:CGRectMake(0.0, 20.0, self.tabsView.contentSize.width ,400)];
        tabLine.tag = kTabLineTag;
        [self.tabsView addSubview:tabLine];
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    TabView *tabView = (TabView*)[self.tabs objectAtIndex:index];
    [tabView setIndicatorColor:self.indicatorColor];
